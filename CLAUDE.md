...
- **Validation backend (PR1)**: post-task validation runs through the execution layer as a second `ExecutionRequest` (`validation_backend: local | same | <name>`, default **`local`**). Non-local (docker/named-local) validation is durable — its own `execution_id` + `execution_phase='validation'` handle, recovered by the Docker probe (`VALIDATING` recovery prefers the validation handle, falls back to a still-open task handle). An SSH validation target **fails loud at scheduler start** (`check_validation_backends`); `maestro validate` is Mode-2 only and is not wired to this check. PR2 adds SSH validation (fresh remote layout, real `CollectPolicy(none)` no-op in `SshTaskHandle`) and then flips the default to `same` (release-noted). This supersedes the "validation still runs locally / `validation_backend` deferred" note above for Mode-1 local/docker.
