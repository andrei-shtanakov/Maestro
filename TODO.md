...
- [ ] **#166 collateral-workstream-kill: per-workstream quarantine и resume без regen** (P2, проектировать можно параллельно, внедрять последним) @owner:github:andrei-shtanakov @id:collateral-workstream-kill
      ...
      разморозкой. Следствие: **новой таблицы не нужно** — NEEDS_REVIEW уже
      останавливает диспетч, поэтому quarantine = один CAS-переход, атомарный
      по построению. Остальные решения: захват архива на recovery до cleanup/
      rework/dispatch, идемпотентный, с меткой источника; численного потолка
      продолжений нет (считать и предупреждать). Осталась реализация.
