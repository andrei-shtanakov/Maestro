...
- [ ] **R-05 follow-up — scheduler-driven e2e**: настоящий e2e через `scheduler.py` с mock spawner (ASSIGN/HOLD/REJECT-путь + retry-gating с decision_id guard), не только прямой `arbiter_client` вызов
